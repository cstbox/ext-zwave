...
""" fibaro motion sensor low-level interface.
...
REG_LUMINISCENCE = ZWaveRegister(0,49,3)
REG_ALARM = ZWaveRegister(0,156,0)
REG_MOTION = ZWaveRegister(0,48,1)


class MOTION_SENSORInstrument(Loggable):
    ...
    def poll(self):
        ...
        raw_temperature = self.read_freg(REG_TEMPERATURE)
        raw_luminiscence = self.read_freg(REG_LUMINISCENCE)
        raw_alarm = self.read_alarm_reg(REG_ALARM)
        raw_motion = self.read_motion_reg(REG_MOTION)

        return self.OutputValues(
                temperature = raw_temperature,
                luminiscence = raw_luminiscence,
                alarm = raw_alarm,
                motion = raw_motion
        )
