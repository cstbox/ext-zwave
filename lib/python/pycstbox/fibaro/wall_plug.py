...
""" fibaro wall plug low-level interface.
...
REG_ENERGY = ZWaveRegister(0,50,0)
REG_AVG_POWER = ZWaveRegister(0,50,2)
REG_POWER = ZWaveRegister(0,49,4)


class WALL_PLUGInstrument(Loggable):
    ...
    def poll(self):
        ...
        raw_energy = self.read_reg(REG_ENERGY)
        raw_power = self.read_reg(REG_POWER)
        raw_avgpower = self.read_reg(REG_AVG_POWER)

        return self.OutputValues(
                energy = raw_energy,
                power = raw_power,
                avgpower = raw_avgpower
        )
